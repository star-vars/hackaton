...
@CheckOrder(1)
public class ProjectHasArchitectureCheck extends SimpleCheck {
    @Override
    public CheckResult performCheck(int iteration, SessionData sessionData) {
        try {
            ...
            List<RepositoryContents> contents = contentsService.getContents(RepositoryId.create(sessionData.getUser().getLogin(), getProject().getRepo()), ProjectStructure.ARCHITECTURE_FOLDER);
            if (contents.size() > 0) {
    ...
    public String getDescription() {
        return "This check performs validation of existence of files in directory " + ProjectStructure.ARCHITECTURE_FOLDER;
    }
